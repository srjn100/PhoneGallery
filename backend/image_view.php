...
WHERE userID = '$id'";
    ...
    $isql="SELECT imageID,imageName,imagePath,phoneName,insertDate,userName
       FROM images as i
       INNER JOIN phones AS p
       ON i.phoneID=p.phoneID
       INNER JOIN users AS u
       ON i.userID=u.userID
       GROUP BY imageID
       ORDER BY imageName";
    if($ires=mysqli_query($conn,$isql)){
        if(mysqli_num_rows($ires)>0){
            $data1=mysqli_fetch_all($ires);
...
<div class="container">
    ...
    <div class="body">
        ...
        <div class="content">
            <div class="h1"><h1>IMAGE LIST</h1></div>
            <div class="table">
                ...
                <table>
                    ...
                    foreach ($data1 as $r){
                        ...
                        foreach ($r as $c){
                            if($i==2){
                                $s=substr($c, 3);
                                echo "<td><img src='$s' class='pimg' alt='$r[1]'></td>";
                            }
                            else{
                                echo "<td>".$c."</td>";
                            }
                        ...
                        $s1=substr($r[2], 3);
                        echo "<td>
                                    <!--<a href='#'><span class='action' onclick=\"return confirm('Are u sure to Edit $r[1]');\">EDIT</span></a>-->
                                    <a href='deleteNedit/delete.php?delItem=image&delID=$r[0]&delName=$s1'><span class='action' onclick=\"return confirm('Are u sure to Delete $r[1]');\">DELETE</span></a>                                    
                                </td>";
                        echo "</tbody>";
                    }
                    ?>
